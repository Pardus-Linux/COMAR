...
void
iks_set_mem_funcs (void *(*malloc_func) (size_t size),
...
iks_stack_new (size_t meta_chunk, size_t data_chunk)
{
	...
	len = sizeof (ikstack) + meta_chunk + data_chunk +
		(sizeof (ikschunk) * 2);
	s = iks_malloc (len);
	if (!s)
		return NULL;
	...
	s->data =
		(ikschunk *) ((char *) s + sizeof (ikstack) +
			      sizeof (ikschunk) + meta_chunk);
	s->data->next = NULL;
...
char *
iks_stack_strcat (ikstack * s, char *old, size_t old_len, const char *src,
		  size_t src_len)
{
	char *ret;
	ikschunk *c;
...
iksparser *
iks_sax_extend (ikstack * s, void *user_data, iksTagHook * tagHook,
		iksCDataHook * cdataHook, iksDeleteHook * deleteHook)
{
	iksparser *prs;
...
sax_core (iksparser * prs, char *buf, int len)
{
	...
	while (pos < len)
	{
		...
		case C_CDATA:
			if ('&' == c)
			{
				...
				{
					err = prs->cdataHook (prs->user_data,
							      &buf[old],
							      pos - old);
					if (IKS_OK != err)
						return err;
	...
	err = IKS_OK;
	if (prs->cdataHook
	    && (prs->context == C_CDATA || prs->context == C_SECT_CDATA_C)
	    && old < pos)
...
iks_new_within (const char *name, ikstack * s)
{
	...
	if (name)
		len = sizeof (struct iks_tag);
	else
		len = sizeof (struct iks_cdata);
...
iks_insert_cdata (iks * x, const char *data, size_t len)
{
	...
	{
		IKS_CDATA_CDATA (y) =
			iks_stack_strcat (x->s, IKS_CDATA_CDATA (y),
					  IKS_CDATA_LEN (y), data, len);
		IKS_CDATA_LEN (y) += len;
	}
	else
	{
...
iks_insert_attrib (iks * x, const char *name, const char *value)
{
	...
	while (y)
	{
		if (strcmp (name, IKS_ATTRIB_NAME (y)) == 0)
			break;
		y = y->next;
	}
	if (NULL == y)
	...
	else
	{
		...
		if (y->prev)
			y->prev->next = y->next;
		if (IKS_TAG_ATTRIBS (x) == y)
			IKS_TAG_ATTRIBS (x) = y->next;
		if (IKS_TAG_LAST_ATTRIB (x) == y)
			IKS_TAG_LAST_ATTRIB (x) = y->prev;
	}
...
iks_hide (iks * x)
{
	...
	if (y)
	{
		if (IKS_TAG_CHILDREN (y) == x)
			IKS_TAG_CHILDREN (y) = x->next;
		if (IKS_TAG_LAST_CHILD (y) == x)
			IKS_TAG_LAST_CHILD (y) = x->prev;
	}
}
...
iks_find (iks * x, const char *name)
{
	...
	while (y)
	{
		if (IKS_TAG == y->type && IKS_TAG_NAME (y)
		    && strcmp (IKS_TAG_NAME (y), name) == 0)
			return y;
		y = y->next;
	}
	return NULL;
...
iks_find_attrib (iks * x, const char *name)
{
	...
	while (y)
	{
		if (IKS_ATTRIB_NAME (y)
		    && strcmp (IKS_ATTRIB_NAME (y), name) == 0)
...
iks *
iks_find_with_attrib (iks * x, const char *tagname, const char *attrname,
		      const char *value)
{
	iks *y;
	...
	if (tagname)
	{
		for (y = IKS_TAG_CHILDREN (x); y; y = y->next)
		{
			if (IKS_TAG == y->type
			    ...
			    && iks_strcmp (iks_find_attrib (y, attrname),
					   value) == 0)
			{
				return y;
			}
...
{
	if (x && IKS_TAG == x->type && IKS_TAG_CHILDREN (x))
		return 1;
	return 0;
}
...
escape (char *dest, char *src, size_t len)
{
	...
	for (i = 0; i < len; i++)
	{
		...
		{
			if (i - j > 0)
				dest = my_strcat (dest, src + j, i - j);
			j = i + 1;
			switch (c)
			{
			case '&':
				dest = my_strcat (dest, "&amp;", 5);
				break;
			case '\'':
				dest = my_strcat (dest, "&apos;", 6);
				break;
			case '"':
				dest = my_strcat (dest, "&quot;", 6);
				break;
			case '<':
				dest = my_strcat (dest, "&lt;", 4);
				break;
			case '>':
				dest = my_strcat (dest, "&gt;", 4);
				break;
...
iks_string (ikstack * s, iks * x)
{
	...
	if (x->type == IKS_CDATA)
	{
		...
		{	
			...
			return iks_stack_strdup (s, IKS_CDATA_CDATA (x),
						 IKS_CDATA_LEN (x));
		}
		else
		{
	...
	while (1)
	{
		if (dir == 0)
		{
			if (y->type == IKS_TAG)
			{
				...
				{
					size += 4 +
						strlen (IKS_ATTRIB_NAME (z)) +
						escape_size (IKS_ATTRIB_VALUE
							     (z),
							     strlen
							     (IKS_ATTRIB_VALUE
							      (z)));
			...
			{
				size += escape_size (IKS_CDATA_CDATA (y),
						     IKS_CDATA_LEN (y));
			}
		}
		z = y->next;
	...
	if (s)
		ret = iks_stack_alloc (s, size + 1);
	else
		ret = iks_malloc (size + 1);
		...
		if (dir == 0)
		{
			if (x->type == IKS_TAG)
			{
				*t++ = '<';
				...
				{
					*t++ = ' ';
					t = my_strcat (t, IKS_ATTRIB_NAME (y),
						       0);
					*t++ = '=';
					*t++ = '\'';
					t = escape (t, IKS_ATTRIB_VALUE (y),
						    strlen (IKS_ATTRIB_VALUE
							    (y)));
					*t++ = '\'';
					y = y->next;
				}
			...
			{
				endofcdata = (IKS_CDATA_LEN (x)) - 1;
				while (endofcdata && (IKS_CDATA_CDATA (x)[endofcdata] == ' ' || IKS_CDATA_CDATA (x)[endofcdata] == 9 || IKS_CDATA_CDATA (x)[endofcdata] == 10)) {
					endofcdata--;					
				}
				if (endofcdata > 0) 
					t = escape (t, IKS_CDATA_CDATA (x),
					    endofcdata + 1);
	...
	while (1)
	{
		if (dir == 0)
		{
			if (x->type == IKS_TAG)
			{
				if (copy == NULL)
				{
					copy = iks_new_within (IKS_TAG_NAME
							       (x), s);
					cur = copy;
				}
				else
			...
			{
				iks_insert_cdata (cur, IKS_CDATA_CDATA (x),
						  IKS_CDATA_LEN (x));
			}
		}
		y = x->next;
...
iks_copy (iks * x)
{
	return iks_copy_within (x,
				iks_stack_new (sizeof (struct iks_tag) * 6,
					       256));
}
...
tagHook (struct dom_data *data, char *name, char **atts, int type)
{
	...
	if (IKS_OPEN == type || IKS_SINGLE == type)
	{
		...
		{
			ikstack *s;
			s = iks_stack_new (data->chunk_size,
					   data->chunk_size);
			x = iks_new_within (name, s);
		}
		if (atts)
